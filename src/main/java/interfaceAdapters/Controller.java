...
public class Controller {
     ...
     * - the first player is the real player and the rest three players are AI
     * (types)
     */
    // TODO: feel free to change the type of the three variables below to list if
    // needed
    // I'm not sure if coming up with a list of players and cards count as
    // dependency to the entity layer
    // If is, we still need to make subtle change on that to make it adhere to clean
    // architecture
    /**
    ...
    private List<Player> regularPlayerList() {
        ...
        for (int k = 1; k < playerNames.size(); k++) {
            player = new BotPlayer(playerNames.get(k), botLevels.get(k - 1)) {// HERE BOT PLAYER
            };
            playerList.add(player);
        }
        return playerList;
     ...
     *
     * playCard()
     * I will give a string that represents the card, which will be in the form of
     * "value-colour"
     * For example: g
     */
     ...
     * 
     * @param colour the colour that the player chooses after a wild card or plus
     *               four card is played
     */
